package com.mybatis.plus.mapper.mapperhelper;
...
public class SqlHelper {
	...
	public static String getDynamicTableName(Class<?> entityClass, String tableName) {
		if (IDynamicTableName.class.isAssignableFrom(entityClass)) {
			...
			sql.append("<choose>");
			sql.append(
					"<when test=\"@com.mybatis.plus.mapper.util.OGNL@isDynamicParameter(_parameter) and dynamicTableName != null and dynamicTableName != ''\">");
	...
	public static String getDynamicTableName(Class<?> entityClass, String tableName, String parameterName) {
		if (IDynamicTableName.class.isAssignableFrom(entityClass)) {
			if (StringUtil.isNotEmpty(parameterName)) {
				StringBuilder sql = new StringBuilder();
				sql.append("<choose>");
				sql.append("<when test=\"@com.mybatis.plus.mapper.util.OGNL@isDynamicParameter(" + parameterName
	...
	public static String getIfIsNullDefaultValue(String entityName, EntityColumn column, String contents,
			boolean empty) {
		StringBuilder sql = new StringBuilder();
		sql.append("<if test=\"");
		if (StringUtil.isNotEmpty(entityName)) {
		...
		sql.append("</if>");
		sql.append(getIfNotNull(entityName, column, contents, empty));
		return sql.toString();
	}
	...
	public static String exampleSelectColumns(Class<?> entityClass) {
		StringBuilder sql = new StringBuilder();
		sql.append("<choose>");
		sql.append("<when test=\"@com.mybatis.plus.mapper.util.OGNL@hasSelectColumns(_parameter)\">");
	...
	public static String exampleCountColumn(Class<?> entityClass) {
		StringBuilder sql = new StringBuilder();
		sql.append("<choose>");
		sql.append("<when test=\"@com.mybatis.plus.mapper.util.OGNL@hasCountColumn(_parameter)\">");
		...
		sql.append("</choose>");
		// 不支持指定列的时候查询全部列
		sql.append("<if test=\"@com.mybatis.plus.mapper.util.OGNL@hasNoSelectColumns(_parameter)\">");
		sql.append(getAllColumns(entityClass));
		sql.append("</if>");
		return sql.toString();
	}
	...
	public static String exampleForUpdate() {
		StringBuilder sql = new StringBuilder();
		sql.append("<if test=\"@com.mybatis.plus.mapper.util.OGNL@hasForUpdate(_parameter)\">");
	...
	public static String exampleCheck(Class<?> entityClass) {
		StringBuilder sql = new StringBuilder();
		sql.append(
				"<bind name=\"checkExampleEntityClass\" value=\"@com.mybatis.plus.mapper.util.OGNL@checkExampleEntityClass(_parameter, '");
	...
	public static String exampleWhereClause() {
		return "<if test=\"_parameter != null\">" + "<where>\n" + "<trim prefixOverrides=\"and |or \">\n"
				...
				+ "    ${@com.mybatis.plus.mapper.util.OGNL@andOr(criteria)} "
				+ "    <if test=\"criteria.valid\">\n"
				+ "      <trim prefix=\"(\" prefixOverrides=\"and |or \" suffix=\")\">\n"
				+ "        <foreach collection=\"criteria.criteria\" item=\"criterion\">\n" + "          <choose>\n"
				+ "            <when test=\"criterion.noValue\">\n"
				+ "              ${@com.mybatis.plus.mapper.util.OGNL@andOr(criterion)} ${criterion.condition}\n"
				+ "            </when>\n" + "            <when test=\"criterion.singleValue\">\n"
				+ "              ${@com.mybatis.plus.mapper.util.OGNL@andOr(criterion)} ${criterion.condition} #{criterion.value}\n"
				+ "            </when>\n" + "            <when test=\"criterion.betweenValue\">\n"
				+ "              ${@com.mybatis.plus.mapper.util.OGNL@andOr(criterion)} ${criterion.condition} #{criterion.value} and #{criterion.secondValue}\n"
